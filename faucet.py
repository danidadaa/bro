    ...
    if len(sys.argv) > 1:
        try:
            ...
            if cycles <= 0:
                ...
                sys.exit(1)
            print(f"{Fore.GREEN}[✓] Will run {cycles} cycles with 1 wallets each{Style.RESET_ALL}")
            return cycles
        except ValueError:
        ...
        while True:
            try:
                cycles = int(input(f"{Fore.YELLOW}[i] Enter the number of cycles (each cycle creates 1 wallets): {Style.RESET_ALL}"))
                if cycles <= 0:
                    print(f"{Fore.RED}[x] Number of cycles must be greater than 0{Style.RESET_ALL}")
                    continue
                print(f"{Fore.GREEN}[✓] Will run {cycles} cycles with 1 wallets each{Style.RESET_ALL}")
